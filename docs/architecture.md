...
- Current CRM tab change: the live sidebar slot previously labeled WhatsApp now serves the Application tab flow in `Index.html`, with a chooser screen and documented open-in-new-tab launcher fallback replacing the old embedded WhatsApp `srcdoc` app.
