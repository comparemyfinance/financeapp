...
function invalidateRowIndexCache_() {
  try {
    CacheService.getScriptCache().remove(cacheKey_("rowIndex"));
  } catch (e) {}
}
...
function findRowForDeal_(sheet, headers, id, vrnKey) {
  ...
  if (!rowNum && vrnKey && c.vrnMap && c.vrnMap[vrnKey])
    rowNum = c.vrnMap[vrnKey];
  ...
  if (rowNum) {
    ...
    try {
      const idCell = String(
        sheet.getRange(rowNum, ID_COL, 1, 1).getValue() || "",
      ).trim();
      const vrnCell = normVrn_(
        sheet.getRange(rowNum, VRN_COL, 1, 1).getValue(),
      );
...
function mergeInto_(existingObj, patchObj) {
  ...
  try {
    if (Session.getActiveUser)
      base.updatedBy = Session.getActiveUser().getEmail();
  } catch (_) {}
  return base;
}
    ...
    if (action === "getFolderFiles") {
      ...
      if (!folderId && payload && typeof payload === "object") {
        folderId =
          ...
          (payload.node &&
            (payload.node.id ||
              payload.node.folderId ||
              payload.node.driveFolderId)) ||
          "";
      }
...
function saveDeal_(sheet, deal) {
  ...
  const vrnSpaced = (deal.vrn || deal.VRN || "")
    .toString()
    .trim()
    .toUpperCase();
  const sourceSheetKey = String(
    deal.sourceSheet || deal.sourcesheet || deal.SourceSheet || "",
  ).trim();

  if (!id && !vrnKey && !sourceSheetKey)
    throw new Error("Missing deal.id (or vrn/sourceSheet fallback)");
  ...
  if (!rowNum) {
    throw new Error(
      'No matching deal found for id "' +
        id +
        '"' +
        (vrnKey ? ' or vrn "' + vrnKey + '"' : "") +
        (sourceSheetKey ? ' or sourceSheet "' + sourceSheetKey + '"' : ""),
    );
  }
  ...
  if (ENABLE_OPTIMISTIC_LOCK && deal.lastUpdated && existingObj.updatedAt) {
    ...
    if (!canBypassSelfConflict && serverT > clientT + 15000) {
      throw new Error(
        "CONFLICT: This deal was updated by " +
          (lastUser || "another user") +
          ". Please refresh.",
      );
    }
  }
  ...
  if (String(existingObj.id || "").trim())
    merged.id = String(existingObj.id).trim();
...
function batchUpdate_(sheet, deals) {
  ...
  deals.forEach((d) => {
    ...
    const ss = String(
      d.sourceSheet || d.sourcesheet || d.SourceSheet || "",
    ).trim();

    let row = null;

    if (id && idx.idToRow.has(id)) row = idx.idToRow.get(id);
    else if (vrn && idx.vrnToRow && idx.vrnToRow.has(vrn))
      row = idx.vrnToRow.get(vrn);
    else if (ss && idx.sourceSheetToRow && idx.sourceSheetToRow.has(ss))
      row = idx.sourceSheetToRow.get(ss);
    ...
    if (String(existing.id || "").trim())
      merged.id = String(existing.id).trim();
    else if (id) merged.id = id;
    else merged.id = vrn;
...
function dailyArchive() {
  ...
  try {
    ...
    const sIdx = h.indexOf("dealstage"),
      dIdx = h.indexOf("updatedat");
    if (sIdx === -1) return;
    const keep = [data[0]],
      move = [];
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      if ((stage === "Completed" || stage === "Not Taken Up") && date < cutoff)
        move.push(row);
      else keep.push(row);
    }
    if (move.length)
      arc
        .getRange(arc.getLastRow() + 1, 1, move.length, data[0].length)
        .setValues(move);
    if (move.length) {
      src.clearContents();
      src.getRange(1, 1, keep.length, data[0].length).setValues(keep);
    }
  } finally {
    lock.releaseLock();
  }
    ...
    if (out && typeof out.getContent === "function") {
      try {
        ...
        return {
          ...
          details: "Handler returned a TextOutput that could not be parsed.",
        };
      }
    }
    return out;
...
function handleDealLocking_(req) {
  ...
  const dealId = String(
    payload.dealId || payload.id || payload.dealID || "",
  ).trim();
  ...
  const clientId =
    String(payload.clientId || payload.clientID || payload.cid || "").trim() ||
    "anon";
  ...
  try {
    ...
    if (
      (action === "acquireLock" || action === "heartbeatLock") &&
      ENABLE_CACHE_LOCKS
    ) {
      ...
      return {
        success: true,
        status: currentOwner ? "RENEWED" : "ACQUIRED",
        dealId: dealId,
        ttlSeconds: ttl,
      };
    }
    ...
    if (action === "releaseLock" && ENABLE_CACHE_LOCKS) {
      const currentOwner = cache.get(lockKey);
      if (!currentOwner)
        return { success: true, status: "NOT_LOCKED", dealId: dealId };
...
function getLogSheet_() {
  ...
  if (sh.getLastRow() === 1) {
    ...
    if (missing.length) {
      sh.getRange(1, sh.getLastColumn() + 1, 1, missing.length).setValues([
        missing,
      ]);
    }
  }
...
function getJigsawToken_() {
  ...
  if (!username || !password)
    throw new Error(
      "Missing JIGSAW_USERNAME / JIGSAW_PASSWORD in Script Properties",
    );
  ...
  if (!json.access_token)
    throw new Error("Jigsaw token response missing access_token");

  // Token expires quickly (~299s). Cache for a bit less.
  const ttlSeconds = Math.max(
    60,
    Math.min(240, Number(json.expires_in || 240)),
  );
...
function validateJigsawAction_(payload) {
  ...
  appendJigsawLog_("validate", {
    ...
    message:
      res.status >= 200 && res.status < 300 ? "Validated" : "Validation failed",
    request: referral,
    response: parsed,
  });
...
function submitJigsawAction_(payload) {
  ...
  const referral = buildJigsawReferralFromDeal_(deal, draft);
  const alwaysValidate = boolProp_(
    "JIGSAW_ALWAYS_VALIDATE_BEFORE_SUBMIT",
    true,
  );
  ...
  appendJigsawLog_("submit", {
    ...
    message:
      s.status >= 200 && s.status < 300 ? "Submitted" : "Submission failed",
    request: referral,
    response: sParsed,
  });
...
function upsertByIntroducerRef_(sheetName, introducerRef, rowObj) {
  ...
  const introVals = sh
    .getRange(2, introCol, lastRow - 1, 1)
    .getValues()
...
function routeJigsawMessageType_(
  msgType,
  introducerRef,
  jigsawRef,
  evtTimeUtc,
  ...
  if (
    t === "further information required" ||
    t === "further info required" ||
    nospace === "furtherinformationrequired"
  ) {
    try {
      fetchAndStoreOpenRequests_(introducerRef, jigsawRef, msgType, evtTimeUtc);
    } catch (e) {
      appendJigsawLog_("fetch_open_requests_failed", {
  ...
  if (t === "documents available" || nospace === "documentsavailable") {
    try {
      fetchAndStoreAvailableDocuments_(
        introducerRef,
        jigsawRef,
        msgType,
        evtTimeUtc,
      ...
      appendJigsawLog_("fetch_documents_failed", {
        introducerReference: introducerRef,
        httpStatus: 0,
        ok: false,
        message: String(e && e.message ? e.message : e),
  ...
  if (
    t === "proofs required" ||
    t === "proof required" ||
    nospace === "proofsrequired" ||
    nospace === "proofrequired"
  ) {
    try {
      fetchAndStoreProofRequirements_(
        introducerRef,
        jigsawRef,
        msgType,
        evtTimeUtc,
      ...
      appendJigsawLog_("fetch_proof_requirements_failed", {
        introducerReference: introducerRef,
        httpStatus: 0,
        ok: false,
        message: String(e && e.message ? e.message : e),
...
function fetchAndStoreOpenRequests_(
  introducerRef,
  jigsawRef,
  msgType,
  evtTimeUtc,
) {
  const path = getJigsawPath_(
    "JIGSAW_OPEN_REQUESTS_PATH",
    DEFAULT_JIGSAW_OPEN_REQUESTS_PATH,
  );
  const q =
    (path.indexOf("?") === -1 ? "?" : "&") +
    "introducerReference=" +
    encodeURIComponent(introducerRef);
...
function fetchAndStoreProofRequirements_(
  introducerRef,
  jigsawRef,
  msgType,
  evtTimeUtc,
  ...
  const path = getJigsawPath_(
    "JIGSAW_PROOF_REQUIREMENTS_PATH",
    DEFAULT_JIGSAW_PROOF_REQUIREMENTS_PATH,
  );
  const q =
    (path.indexOf("?") === -1 ? "?" : "&") +
    "introducerReference=" +
...
function respondProofRequirement_(payload) {
  ...
  const path = getJigsawPath_(
    "JIGSAW_RESPOND_PROOF_REQUIREMENT_PATH",
    DEFAULT_JIGSAW_RESPOND_PROOF_REQUIREMENT_PATH,
  );
...
function respondToFurtherInfoRequest_(payload) {
  ...
  const requestId =
    ...
    (payload.requestId ||
      payload.RequestId ||
      payload.furtherInformationRequestId);
  if (!introducerRef) throw new Error("Missing introducerReference");
  if (!requestId) throw new Error("Missing requestId");

  const path = getJigsawPath_(
    "JIGSAW_RESPOND_REQUEST_PATH",
    DEFAULT_JIGSAW_RESPOND_REQUEST_PATH,
  );
...
function fetchAndStoreAvailableDocuments_(
  introducerRef,
  jigsawRef,
  msgType,
  evtTimeUtc,
  ...
  const path = getJigsawPath_(
    "JIGSAW_AVAILABLE_DOCUMENTS_PATH",
    DEFAULT_JIGSAW_AVAILABLE_DOCUMENTS_PATH,
  );
  const q =
    (path.indexOf("?") === -1 ? "?" : "&") +
    "introducerReference=" +
...
function fetchDocument_(payload) {
  const introducerRef = resolveIntroducerReference_(payload);
  const documentId =
    payload && (payload.documentId || payload.DocumentId || payload.id);
  if (!introducerRef) throw new Error("Missing introducerReference");
  if (!documentId) throw new Error("Missing documentId");

  const path = getJigsawPath_(
    "JIGSAW_GET_DOCUMENT_PATH",
    DEFAULT_JIGSAW_GET_DOCUMENT_PATH,
  );
  const q =
    (path.indexOf("?") === -1 ? "?" : "&") +
    "introducerReference=" +
...
function uploadDocumentResponse_(payload) {
  ...
  const path = getJigsawPath_(
    "JIGSAW_UPLOAD_DOCUMENT_RESPONSE_PATH",
    DEFAULT_JIGSAW_UPLOAD_DOCUMENT_RESPONSE_PATH,
  );
...
function getJigsawOpenRequestsAction_(payload) {
  try {
    const introducerRef = resolveIntroducerReference_(payload);
    if (!introducerRef)
      return { success: false, error: "Missing introducerReference/dealId" };

    const out = fetchAndStoreOpenRequests_(
      introducerRef,
      "",
      "manual_fetch",
      "",
...
function getJigsawProofRequirementsAction_(payload) {
  try {
    ...
    const out = fetchAndStoreProofRequirements_(
      introducerRef,
      "",
      "manual_fetch",
      "",
...
function getJigsawAvailableDocumentsAction_(payload) {
  try {
    ...
    const out = fetchAndStoreAvailableDocuments_(
      introducerRef,
      "",
      "manual_fetch",
      "",
...
function handleJigsawWebhook_(e, bodyObj, rawBody) {
  ...
  if (!secret) {
    appendJigsawLog_("webhook_rejected", {
      httpStatus: 500,
      ok: false,
      message: "Missing JIGSAW_SHARED_SECRET",
    });
  ...
  if (!sig) {
    ...
    appendJigsawLog_("webhook_rejected", {
      httpStatus: 401,
      ok: false,
      message: "Missing signature (sig)",
      response: bodyObj,
    });
    return {
      success: false,
      error:
        "Missing signature. Provide as query param sig=... (or JSON JfSignature).",
    };
  }
  ...
  const ok =
    equalsSig_(sigNorm, expectedHex) || equalsSig_(sigNorm, expectedB64);
  ...
  const introducerRef = String(
    bodyObj.IntroducerReference || bodyObj.introducerReference || "",
  ).trim();
  const jigsawRef = bodyObj.JigsawReference || bodyObj.jigsawReference || "";
  const msgType = String(
    bodyObj.MessageType || bodyObj.messageType || "",
  ).trim();
  const evtTime = String(
    bodyObj.EventTimeUtc || bodyObj.eventTimeUtc || "",
  ).trim();

  // MessageType-specific follow-up (best-effort): fetch open requests / documents lists and store to dedicated sheets.
  try {
    routeJigsawMessageType_(
      msgType,
      introducerRef,
      jigsawRef,
      evtTime,
    ...
    appendJigsawLog_("webhook_followup_failed", {
      introducerReference: introducerRef,
      ok: false,
  ...
  withLock_(20000, () => {
    const sheet = getSheet_();
    const dealId =
      findDealIdByIntroducerReference_(sheet, introducerRef) || introducerRef;
...
function getInboundSignature_(e, bodyObj) {
  ...
  return (
    bodyObj.JfSignature || bodyObj.jfSignature || bodyObj.signature || null
  );
}
...
function updateDealFields_(sheet, dealId, fields) {
  ...
  sheet
    ...
    .setValues([objectToRow_(headers, merged)]);
}
...
function buildJigsawReferralFromDeal_(deal, draft) {
  ...
  // Applicant basics
  const titleVal = mapTitle_(
    pick_(fd, ["title", "Title", "applicantTitle", "salutation"]),
  );
  const genderVal = mapGender_(pick_(fd, ["gender", "Gender"]), titleVal);
  const firstName = str_(
    pick_(fd, ["firstName", "FirstName", "forename", "forename1"]),
  );
  const middleNames = str_(pick_(fd, ["middleNames", "MiddleNames"]));
  const lastName = str_(pick_(fd, ["lastName", "LastName", "surname"]));
  const dobIso = normalizeDateIso_(
    pick_(fd, ["dob", "dateOfBirth", "DateOfBirth", "DOB"]),
  );
  const maritalStatus = mapMaritalStatus_(
    pick_(fd, ["maritalStatus", "MaritalStatus"]),
  );
  ...
  let incomeFreq = mapIncomeFrequencyType_(
    pick_(fd, [
      "incomeFrequencyType",
      "incomeFrequency",
      "IncomeFrequencyType",
  ...
  const mobile = normalizeUkPhone_(
    pick_(fd, ["mobile", "Mobile", "phone", "Phone"]),
  );
  const email = str_(pick_(fd, ["email", "Email"]));

  const currentAddr = buildResidentialAddress_(fd, "current");
  const totalMonthsAtCurrent =
    num_(currentAddr.YearsAtAddress, 0) * 12 +
    num_(currentAddr.MonthsAtAddress, 0);
  const previousAddr =
    totalMonthsAtCurrent < 36 ? buildResidentialAddress_(fd, "previous") : null;

  // Employment
  const currentEmp = buildEmployment_(fd, "current");
  const totalMonthsAtEmp =
    num_(currentEmp.YearsInEmployment, 0) * 12 +
    num_(currentEmp.MonthsInEmployment, 0);
  const previousEmp =
    totalMonthsAtEmp < 36 ? buildEmployment_(fd, "previous") : null;

  // Server-side UAT safety: durations must not be 0, and history must cover >= 36 months where required.
  if (totalMonthsAtCurrent < 1)
    throw new Error(
      "CurrentAddress: YearsAtAddress/MonthsAtAddress must total at least 1 month.",
    );
  if (totalMonthsAtCurrent < 36) {
    if (!previousAddr)
      throw new Error(
        "PreviousAddress is required when current address is less than 36 months.",
      );
    const prevMonths =
      num_(previousAddr.YearsAtAddress, 0) * 12 +
      num_(previousAddr.MonthsAtAddress, 0);
    if (prevMonths < 1)
      throw new Error(
        "PreviousAddress: YearsAtAddress/MonthsAtAddress must total at least 1 month.",
      );
  ...
  if (totalMonthsAtEmp < 1)
    throw new Error(
      "CurrentEmployment: YearsInEmployment/MonthsInEmployment must total at least 1 month.",
    );
  if (totalMonthsAtEmp < 36) {
    if (!previousEmp)
      throw new Error(
        "PreviousEmployment is required when current employment is less than 36 months.",
      );
    const prevEmpMonths =
    ...
    if (prevEmpMonths < 1)
      throw new Error(
        "PreviousEmployment: YearsInEmployment/MonthsInEmployment must total at least 1 month.",
      );
  ...
  const src =
    (draft && (draft.ProposalSource || draft.proposalSource)) ||
    fd.proposalSource ||
    null;
  ...
  const targetLender =
    (draft && (draft.TargetLender || draft.targetLender)) || fd.targetLender;
  if (
  ...
  if (!referral.MainApplicant.FirstName)
    missing.push("MainApplicant.FirstName");
  if (!referral.MainApplicant.LastName) missing.push("MainApplicant.LastName");
  if (!referral.MainApplicant.DateOfBirth)
    missing.push("MainApplicant.DateOfBirth");
  ...
  if (!referral.MainApplicant.MaritalStatus)
    missing.push("MainApplicant.MaritalStatus");
  if (
  ...
  if (missing.length)
    throw new Error(
      "Missing required fields for Jigsaw: " + missing.join(", "),
    );
...
function buildResidentialAddress_(fd, which) {
  ...
  const postcode = str_(
    pick_(fd, [prefix + "Postcode", prefix + "PostalCode", prefix + "Zip"]),
  );
  const ym = normalizeYearsMonths_(
    pick_(fd, [
      prefix + "YearsAtAddress",
      prefix + "AddressYears",
      prefix + "Years",
    ]),
    pick_(fd, [
      prefix + "MonthsAtAddress",
      prefix + "AddressMonths",
      prefix + "Months",
    ]),
  );
  const years = ym.years;
  const months = ym.months;
  const resStatus = mapResidentialStatus_(
    pick_(fd, [
      prefix + "ResidentialStatus",
      "residentialStatus",
      "ResidentialStatus",
    ]),
    1,
...
function buildEmployment_(fd, which) {
  ...
  const status = mapEmploymentStatus_(
    pick_(fd, [prefix + "Status", "employmentStatus", "EmploymentStatus"]),
    1,
  );
  const jobTitle = str_(
    pick_(fd, [
  ...
  const company = str_(
    pick_(fd, [prefix + "Company", "companyName", "employerName"]),
  );
  const phone = normalizeUkPhone_(
    pick_(fd, [prefix + "Phone", "workPhone", "employerPhone"]),
  );
  const ym = normalizeYearsMonths_(
    pick_(fd, [prefix + "Years", "yearsAtEmployment", "YearsInEmployment"]),
    pick_(fd, [prefix + "Months", "monthsAtEmployment", "MonthsInEmployment"]),
  );
  const years = ym.years;
  const months = ym.months;
  ...
  const city =
    str_(pick_(fd, [prefix + "City"])) ||
    str_(pick_(fd, ["currentCity", "currentTown"])) ||
    null;
...
function buildVehicleAndFinance_(fd, deal) {
  ...
  const vrm =
    str_(pick_(fd, ["vrn", "VRN", "vrm", "VRM"])) ||
    str_(pick_(deal || {}, ["vrn", "VRN"])) ||
    "";
  const regDate = normalizeDateIso_(
    pick_(fd, ["registrationDate", "RegistrationDate", "regDate"]),
  );
  const capCode = str_(
    pick_(fd, ["capVehicleCode", "CAPVehicleCode", "capCode"]),
  );
  const mileage = num_(
    pick_(fd, [
  ...
  const vin = str_(
    pick_(fd, ["vin", "VIN", "ChassisNumberOrVin", "chassisNumber"]),
  );
  ...
  const modelRaw =
    str_(pick_(fd, ["vehicleModel", "model", "VehicleModel", "Model"])) || "";
  let derivative =
    str_(
      pick_(fd, [
        "vehicleDerivative",
  ...
  if (annualMileage !== null && annualMileage > 0)
    VehicleAndFinance.AnnualMileage = Math.round(annualMileage);

  // Do not send zero/negative values (Jigsaw rejects <= 0)
  if (advance !== null && advance > 0)
    VehicleAndFinance.BasicFinance.Advance = advance;

  // Annual mileage is required by Jigsaw (int, min 1) for most products
  if (annualMileage !== null && annualMileage > 0)
    VehicleAndFinance.AnnualMileage = annualMileage;
...
function mapEmploymentStatus_(v, defVal) {
  ...
  if (s.includes("full") && (s.includes("temp") || s.includes("temporary")))
    return 3;
  if (s.includes("part") && (s.includes("temp") || s.includes("temporary")))
    return 4;
  ...
  try {
    ...
    if (results.length === 0) {
      return {
        success: true,
        folders: [],
        message: q ? "No matches found." : "Root folder is empty.",
      };
    }
    ...
    if (e.message.indexOf("Drive is not defined") !== -1) {
      return {
        success: false,
        error: "CRITICAL: Please add 'Drive API' in Apps Script Services.",
      };
    }
